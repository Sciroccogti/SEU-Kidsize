...
int main(int argc, char **argv)
{
    ...
    ros::ServiceServer boneSrv = node.advertiseService("/bonelength", GetBoneLengthService);
    ros::ServiceServer IKSrv = node.advertiseService("/kinematics", InverseKinematicsService);
    ros::ServiceServer aqSizeSrv = node.advertiseService("/queuesize", GetQueueSizeService);
    ...
    ros::Publisher imuPublisher = node.advertise<ImuData>("/imu", 1);
    ...
    while(ros::ok())
    {
        if(mcu->isOpen())
        {
            ...
            {
                req_cnt++;
                imudata.stamp = ros::Time::now().nsec;
...
void NormalizeIMU(ImuData &idata)
{
    ...
    if(idata.pitch<-30.0) idata.fall = ImuData::FALL_BACKWARD;
    else if(idata.pitch>40.0) idata.fall = ImuData::FALL_FORWARD;
    else if(idata.roll<-40.0) idata.fall = ImuData::FALL_RIGHT;
    else if(idata.roll>40.0) idata.fall = ImuData::FALL_LEFT;
    else idata.fall = ImuData::FALL_NONE;
}
