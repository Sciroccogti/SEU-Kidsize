...
void Serial::clearPort()
{
  tcflush(socket_fd_, TCIFLUSH);
}

void Serial::setPortName(const char *port_name)
{
  strcpy(port_name_, port_name);
}

char *Serial::getPortName()
{
  return port_name_;
}
...
int Serial::readPort(uint8_t *packet, int length)
{
  return read(socket_fd_, packet, length);
}

int Serial::writePort(uint8_t *packet, int length)
{
  return write(socket_fd_, packet, length);
}

void Serial::setPacketTimeout(uint16_t packet_length)
{
  packet_start_time_  = getCurrentTime();
  packet_timeout_     = (tx_time_per_byte * (double)packet_length) + (LATENCY_TIMER * 2.0) + 2.0;
}

void Serial::setPacketTimeout(double msec)
{
  packet_start_time_  = getCurrentTime();
  packet_timeout_     = msec;
}

bool Serial::isPacketTimeout()
{
...
double Serial::getTimeSinceStart()
{
  double time;
...
bool Serial::setupPort(int cflag_baud)
{
  ...
  if(socket_fd_ < 0)
  {
    printf("[Serial::SetupPort] Error opening serial port!\n");
    return false;
  }
...
bool Serial::setCustomBaudrate(int speed)
{
  // try to set a custom divisor
  struct serial_struct ss;
  if(ioctl(socket_fd_, TIOCGSERIAL, &ss) != 0)
  {
    printf("[Serial::SetCustomBaudrate] TIOCGSERIAL failed!\n");
    return false;
  }
  ...
  {
    printf("[Serial::SetCustomBaudrate] Cannot set speed to %d, closest is %d \n", speed, closest_speed);
    return false;
  }

  if(ioctl(socket_fd_, TIOCSSERIAL, &ss) < 0)
  {
    printf("[Serial::SetCustomBaudrate] TIOCSSERIAL failed!\n");
    return false;
  }
...
int Serial::getCFlagBaud(int baudrate)
{
  switch(baudrate)
  {
    case 9600:
