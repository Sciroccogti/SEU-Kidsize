...
#define poses_key_ "poses"
    ...
    bool parse(std::string act_file, ActMap &acts, PosMap &poses)
    {
        ...
        for (auto &pos : pos_pt)
        {
            ...
            {
                t_pos.pose_info[robot::get_motion_by_name(info.first)]
                        = get_pose_from_string(info.second.data());
            }
    ...
    Robot::Robot(const std::string &robot_file, const std::string &offset_file)
    {
        ...
        if (!get_tree_from_file(robot_file, pt)) return ;
        main_bone_ = parse_bone(pt, bone_map_, joint_map_);

        if (main_bone_.get()==nullptr) return;
    ...
    TransformMatrix Robot::leg_forward_kinematics(vector<double> degs, bool left)
    {
        ...
        body = foot * TransformMatrix(0, 0, foot_length) * TransformMatrix(-degs[5], 'x') 
                ...
                * TransformMatrix(-degs[2], 'y') * TransformMatrix(-degs[1], 'x') 
                * TransformMatrix(-degs[0], 'z') 
