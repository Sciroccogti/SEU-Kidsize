...
namespace robot
{
    ...
    const std::map<std::string, RobotMotion> namemotion_map = {{"none", MOTION_NONE},
    ...
    public:
        ...
        seumath::TransformMatrix get_foot_mat_from_pose(const RobotPose &pose, bool left);
        seumath::TransformMatrix get_body_mat_from_pose(const RobotPose &pose);

        seumath::TransformMatrix leg_forward_kinematics(std::vector<double> degs, bool left);
        ...
        {
            std::vector<double> res = {joint_map_["jhead1"]->current_deg, joint_map_["jhead2"]->current_deg};
            return res;
        }
