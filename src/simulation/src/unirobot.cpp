...
UniRobot::UniRobot(ros::NodeHandle *node, string name, std::string actdir) : Robot()
{
  ...
  mLedTaskSubscriber = mNode->subscribe("/task/led", 1, &UniRobot::LedTaskUpdate, this);
}
...
void UniRobot::LedTaskUpdate(const common::LedTask::ConstPtr &p)
{
  mLedtask = *p;
}
...
int UniRobot::myStep()
{
  ...
  mHeadMtx.unlock();
  mLEDs[0]->set(0xff0000 * mLedtask.led1);
  mLEDs[1]->set(0xff0000 * mLedtask.led2);
  setPositions();
  checkFall();
  ros::spinOnce();
  totalTime += mTimeStep;
...
void UniRobot::runAct(std::string name)
{
  ...
  for (auto angles : mBodyAngles)
  {
    ...
    {
      mAngles.left_ankle_pitch =
          lastOuts.left_ankle_pitch + i * (angles.left_ankle_pitch - lastOuts.left_ankle_pitch) / angles.time;
      mAngles.left_ankle_roll =
          lastOuts.left_ankle_roll + i * (angles.left_ankle_roll - lastOuts.left_ankle_roll) / angles.time;
      mAngles.left_hip_pitch =
          lastOuts.left_hip_pitch + i * (angles.left_hip_pitch - lastOuts.left_hip_pitch) / angles.time;
      mAngles.left_hip_roll =
          lastOuts.left_hip_roll + i * (angles.left_hip_roll - lastOuts.left_hip_roll) / angles.time;
      ...
      mAngles.right_ankle_pitch =
          lastOuts.right_ankle_pitch + i * (angles.right_ankle_pitch - lastOuts.right_ankle_pitch) / angles.time;
      mAngles.right_ankle_roll =
          lastOuts.right_ankle_roll + i * (angles.right_ankle_roll - lastOuts.right_ankle_roll) / angles.time;
      mAngles.right_hip_pitch =
          lastOuts.right_hip_pitch + i * (angles.right_hip_pitch - lastOuts.right_hip_pitch) / angles.time;
      mAngles.right_hip_roll =
          lastOuts.right_hip_roll + i * (angles.right_hip_roll - lastOuts.right_hip_roll) / angles.time;
      mAngles.right_hip_yaw =
          lastOuts.right_hip_yaw + i * (angles.right_hip_yaw - lastOuts.right_hip_yaw) / angles.time;
      ...
      mAngles.right_shoulder =
          mAngles.right_shoulder + i * (angles.right_shoulder - mAngles.right_shoulder) / angles.time;
...
void UniRobot::runWalk(const Rhoban::IKWalkParameters &params, double timeLength, double &phase, double &time)
{
  ...
  for (double t = 0.0; t <= timeLength; t += 1.0 / engineFrequency)
  {
    ...
    bool success = Rhoban::IKWalk::walk(params,                 // Walk parameters
    ...
    if (!success)
    {
      ...
      std::cout << time << " Inverse Kinematics error: " << params.stepGain << ' ' << params.lateralGain << std::endl;
    }
    else
    {
...
std::vector<BodyAngles> UniRobot::loadAct(std::string name)
{
  ...
  {
    ifs >> angles.time;
    ifs >> angles.right_hip_yaw >> angles.right_hip_roll >> angles.right_hip_pitch >> angles.right_knee >>
        angles.right_ankle_pitch >> angles.right_ankle_roll;
    ifs >> angles.left_hip_yaw >> angles.left_hip_roll >> angles.left_hip_pitch >> angles.left_knee >>
        angles.left_ankle_pitch >> angles.left_ankle_roll;
    ifs >> angles.right_shoulder >> angles.right_elbow >> angles.left_shoulder >> angles.left_elbow;
